...
For real Meta publishing, use a server-storage mode (Supabase/S3-compatible): Meta fetches media via a short-lived signed URL minted directly from the object store (`IMediaService.GetPublishingUrlAsync`). Local-disk mode cannot publish to Meta — `GetPublishingUrlAsync` throws `NotSupportedException` because the old `/api/media/files/{storageKey}` proxy route was removed (media privacy redesign) and there is no signed-URL source; see [docs/public-media-route.md](public-media-route.md). Authenticated previews in the SPA use `GET /api/media/{mediaId}/file` instead.
