...
public class S3CompatibleMediaStorageProvider : IMediaStorageProvider, IDisposable
{
    ...
    {
        // Server-generated bytes (Phase 3 IG derivative) go straight to the bucket via
        // the internal client over the Docker network: no presigned URL, no browser.
        ...
        _logger.LogInformation(
            "Server-side uploaded object {Key} ({ContentType}) to bucket {Bucket}",
