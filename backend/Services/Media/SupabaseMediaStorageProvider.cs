...
public class SupabaseMediaStorageProvider : IMediaStorageProvider, IDisposable
{
    ...
    {
        // Server-generated bytes (Phase 3 IG derivative). Unlike browser uploads (which
        // use the signed-URL flow), these originate on the API, so we PUT directly with
        // the service-role key. x-upsert:true overwrites any existing derivative.
        // POST creates, PUT upserts; we use PUT so retries are idempotent.
        var path = $"{ApiPrefix}/object/{Uri.EscapeDataString(_bucket)}/{EncodePath(storageKey)}";

        using var streamContent = new StreamContent(content);
        streamContent.Headers.ContentType = new MediaTypeHeaderValue(contentType);

        using var req = new HttpRequestMessage(HttpMethod.Put, path) { Content = streamContent };
        ...
        if (!resp.IsSuccessStatusCode)
        {
            var body = await resp.Content.ReadAsStringAsync(cancellationToken);
            _logger.LogWarning(
                "Supabase server-side upload failed for key {Key}: status={Status} body={Body}",
                RedactKey(storageKey), (int)resp.StatusCode, body);
            throw new InvalidOperationException(
                $"Supabase failed to store generated media object (HTTP {(int)resp.StatusCode}).");
        }

        _logger.LogInformation(
            "Server-side uploaded object {Key} ({ContentType}) to Supabase bucket {Bucket}",
            RedactKey(storageKey), contentType, _bucket);
    }
